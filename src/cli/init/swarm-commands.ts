...
## Usage
\`\`\`bash
codex-flow swarm "Research objective" --strategy research --mode distributed --parallel
\`\`\`
...
## Usage
\`\`\`bash
codex-flow swarm "Build application" --strategy development --mode hierarchical --parallel
\`\`\`
...
## Usage
\`\`\`bash
codex-flow swarm "Analyze data" --strategy analysis --parallel --max-agents 10
\`\`\`
...
## Usage
\`\`\`bash
codex-flow swarm "Test application" --strategy testing --mode mesh --parallel
\`\`\`
...
## Usage
\`\`\`bash
codex-flow swarm "Optimize performance" --strategy optimization --mode hybrid --parallel
\`\`\`
...
## Usage
\`\`\`bash
codex-flow swarm "System maintenance" --strategy maintenance --mode centralized --monitor
\`\`\`
...
codex-flow swarm "Research modern web frameworks" --strategy research --mode distributed --parallel --max-agents 6

# Market analysis with coordinated batch operations
codex-flow swarm "Analyze AI market trends" --strategy research --parallel --monitor --timeout 120
\`\`\`

### Development Tasks with Batch Coordination
\`\`\`bash
# Hierarchical development with batch file operations
codex-flow swarm "Build microservice API" --strategy development --mode hierarchical --parallel --max-agents 8

# React dashboard with coordinated component development
codex-flow swarm "Create React dashboard" --strategy development --parallel --monitor --output sqlite
\`\`\`

### Analysis Tasks with Batch Processing
\`\`\`bash
# Mesh-coordinated data analysis
codex-flow swarm "Analyze user behavior data" --strategy analysis --mode mesh --parallel --max-agents 10

# Performance analysis with monitoring
codex-flow swarm "Performance analysis of application" --strategy analysis --monitor --output csv
\`\`\`

### Testing Tasks with Parallel Validation
\`\`\`bash
# Comprehensive parallel testing
codex-flow swarm "Comprehensive testing suite" --strategy testing --parallel --max-agents 12

# Security testing with distributed coordination
codex-flow swarm "Security testing analysis" --strategy testing --mode distributed --monitor
\`\`\`

### Optimization Tasks with Hybrid Coordination
\`\`\`bash
# Database optimization with hybrid approach
codex-flow swarm "Optimize database queries" --strategy optimization --mode hybrid --parallel

# Frontend optimization with batch processing
codex-flow swarm "Frontend performance optimization" --strategy optimization --monitor --max-agents 6
\`\`\`

### Maintenance Tasks with Centralized Control
\`\`\`bash
# Dependency updates with centralized coordination
codex-flow swarm "Update dependencies safely" --strategy maintenance --mode centralized --monitor

# System health checks with batch operations
codex-flow swarm "System health check" --strategy maintenance --parallel --output json
\`\`\`
