...
```bash
npx codex-flow hook pre-command [options]
```
...
```bash
npx codex-flow hook pre-command --command "npm install express"
```

### Safety validation

```bash
npx codex-flow hook pre-command -c "rm -rf node_modules" --validate-safety
```

### Permission check

```bash
npx codex-flow hook pre-command -c "sudo apt update" --check-permissions
```

### Dry run preview

```bash
npx codex-flow hook pre-command -c "git push origin main" --dry-run
```
...
npx codex-flow hook pre-command --command "your command here" --validate-safety
```
