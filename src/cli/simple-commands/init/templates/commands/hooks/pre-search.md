...
```bash
npx codex-flow hook pre-search [options]
```
...
```bash
npx codex-flow hook pre-search --query "authentication implementation"
```

### With caching

```bash
npx codex-flow hook pre-search -q "React hooks usage" --cache-results
```

### Memory check first

```bash
npx codex-flow hook pre-search -q "previous bug fixes" --check-memory
```

### Query expansion

```bash
npx codex-flow hook pre-search -q "auth" --expand-query --suggest-filters
```
...
```bash
# Before searching
npx codex-flow hook pre-search --query "your search" --cache-results --check-memory
```
