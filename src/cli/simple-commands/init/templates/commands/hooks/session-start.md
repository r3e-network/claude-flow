...
```bash
npx codex-flow hook session-start [options]
```
...
```bash
npx codex-flow hook session-start --session-id "dev-session-2024"
```

### With full restoration

```bash
npx codex-flow hook session-start -s "feature-auth" --restore-context --load-preferences
```

### Auto swarm initialization

```bash
npx codex-flow hook session-start -s "bug-fix-789" --init-swarm
```

### Telemetry enabled

```bash
npx codex-flow hook session-start -s "performance-opt" --telemetry
```
...
```bash
# At session start
npx codex-flow hook session-start --session-id "your-session" --restore-context
```
