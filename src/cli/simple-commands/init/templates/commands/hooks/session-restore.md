...
```bash
npx codex-flow hook session-restore [options]
```
...
```bash
npx codex-flow hook session-restore --session-id "dev-session-2024"
```

### Full restoration

```bash
npx codex-flow hook session-restore -s "feature-auth" --load-memory --restore-files --resume-tasks
```

### Selective restore

```bash
npx codex-flow hook session-restore -s "bug-fix-123" --load-memory --resume-tasks
```

### Context merging

```bash
npx codex-flow hook session-restore -s "refactor-api" --merge-context
```
...
```bash
# To restore context
npx codex-flow hook session-restore --session-id "previous-session" --load-memory
```
