...
```bash
npx codex-flow hook post-command [options]
```
...
```bash
npx codex-flow hook post-command --command "npm test" --exit-code 0
```

### With output analysis

```bash
npx codex-flow hook post-command -c "git status" -e 0 --analyze-output
```

### Cache update

```bash
npx codex-flow hook post-command -c "npm list" -e 0 --update-cache
```

### Performance tracking

```bash
npx codex-flow hook post-command -c "build.sh" -e 0 --track-metrics
```
...
npx codex-flow hook post-command --command "npm build" --exit-code 0 --analyze-output
```
