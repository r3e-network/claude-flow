...
```bash
npx codex-flow hook notification [options]
```
...
```bash
npx codex-flow hook notification --message "Completed authentication module"
```

### Warning notification

```bash
npx codex-flow hook notification -m "Potential security issue found" -l warning
```

### Broadcast to swarm

```bash
npx codex-flow hook notification -m "API refactoring started" --broadcast
```

### Decision tracking

```bash
npx codex-flow hook notification -m "Chose JWT over sessions for auth" --memory-store
```
...
```bash
# For coordination
npx codex-flow hook notification --message "Starting database migration" --broadcast --memory-store
```
