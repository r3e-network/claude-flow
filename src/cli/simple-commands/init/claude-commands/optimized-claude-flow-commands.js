...
- \`./codex-flow start --parallel\` - Start with enhanced parallel processing
- \`./codex-flow status\` - Check system status
- \`./codex-flow status --concurrent\` - Check status with parallel monitoring
- \`./codex-flow monitor\` - Real-time monitoring
- \`./codex-flow monitor --performance\` - Enhanced performance monitoring
- \`./codex-flow stop\` - Stop orchestration

### 🤖 Agent Management (Parallel)
- \`./codex-flow agent spawn <type>\` - Create new agent
- \`./codex-flow agent batch-spawn <config>\` - Create multiple agents in parallel
- \`./codex-flow agent list\` - List active agents
- \`./codex-flow agent parallel-status\` - Check all agent status concurrently
- \`./codex-flow agent info <id>\` - Agent details
- \`./codex-flow agent terminate <id>\` - Stop agent
- \`./codex-flow agent batch-terminate <ids>\` - Stop multiple agents in parallel

### 📋 Task Management (Concurrent)
- \`./codex-flow task create <type> "description"\` - Create task
- \`./codex-flow task batch-create <tasks-file>\` - Create multiple tasks in parallel
- \`./codex-flow task list\` - List all tasks
- \`./codex-flow task parallel-status\` - Check all task status concurrently
- \`./codex-flow task status <id>\` - Task status
- \`./codex-flow task cancel <id>\` - Cancel task
- \`./codex-flow task batch-cancel <ids>\` - Cancel multiple tasks in parallel
- \`./codex-flow task workflow <file>\` - Execute workflow
- \`./codex-flow task parallel-workflow <files>\` - Execute multiple workflows concurrently

### 🧠 Memory Operations (Batch Enhanced)
- \`./codex-flow memory store "key" "value"\` - Store data
- \`./codex-flow memory batch-store <entries-file>\` - Store multiple entries in parallel
- \`./codex-flow memory query "search"\` - Search memory
- \`./codex-flow memory parallel-query <queries>\` - Execute multiple queries concurrently
- \`./codex-flow memory stats\` - Memory statistics
- \`./codex-flow memory stats --concurrent\` - Parallel memory analysis
- \`./codex-flow memory export <file>\` - Export memory
- \`./codex-flow memory concurrent-export <namespaces>\` - Export multiple namespaces in parallel
- \`./codex-flow memory import <file>\` - Import memory
- \`./codex-flow memory batch-import <files>\` - Import multiple files concurrently

### ⚡ SPARC Development (Optimized)
- \`./codex-flow sparc "task"\` - Run SPARC orchestrator
- \`./codex-flow sparc parallel "tasks"\` - Run multiple SPARC tasks concurrently
- \`./codex-flow sparc modes\` - List all 17+ SPARC modes
- \`./codex-flow sparc run <mode> "task"\` - Run specific mode
- \`./codex-flow sparc batch <modes> "task"\` - Run multiple modes in parallel
- \`./codex-flow sparc tdd "feature"\` - TDD workflow
- \`./codex-flow sparc concurrent-tdd <features>\` - Parallel TDD for multiple features
...
- \`./codex-flow swarm batch <tasks-config>\` - Multiple swarms in parallel
- \`./codex-flow swarm concurrent "tasks" --parallel\` - Concurrent swarm execution

### 🌍 MCP Integration (Parallel)
- \`./codex-flow mcp status\` - MCP server status
- \`./codex-flow mcp parallel-status\` - Check all MCP servers concurrently
- \`./codex-flow mcp tools\` - List available tools
- \`./codex-flow mcp config\` - Show configuration
- \`./codex-flow mcp logs\` - View MCP logs
- \`./codex-flow mcp batch-logs <servers>\` - View multiple server logs in parallel

### 🤖 Claude Integration (Enhanced)
- \`./codex-flow claude spawn "task"\` - Spawn Claude with enhanced guidance
- \`./codex-flow claude batch-spawn <tasks>\` - Spawn multiple Claude instances in parallel
- \`./codex-flow claude batch <file>\` - Execute workflow configuration

### 🚀 Batchtools Commands (New)
- \`./codex-flow batchtools status\` - Check batchtools system status
- \`./codex-flow batchtools monitor\` - Real-time performance monitoring
- \`./codex-flow batchtools optimize\` - System optimization recommendations
- \`./codex-flow batchtools benchmark\` - Performance benchmarking
- \`./codex-flow batchtools config\` - Batchtools configuration management
...
\`\`\`bash
./codex-flow swarm "Build REST API" --strategy development --monitor --review --parallel
\`\`\`

### Run concurrent TDD workflow:
\`\`\`bash
./codex-flow sparc concurrent-tdd "user authentication,payment processing,notification system"
\`\`\`

### Store project context with batch operations:
\`\`\`bash
./codex-flow memory batch-store "project-contexts.json" --namespace project --parallel
\`\`\`

### Spawn specialized agents in parallel:
\`\`\`bash
./codex-flow agent batch-spawn agents-config.json --parallel --validate
\`\`\`
...
./codex-flow monitor --performance --real-time

# Check resource utilization
./codex-flow batchtools monitor --resources --detailed

# Analyze operation bottlenecks
./codex-flow performance analyze --bottlenecks --optimization
\`\`\`

### Optimization Commands
\`\`\`bash
# Auto-optimize system configuration
./codex-flow batchtools optimize --auto-tune

# Performance benchmarking
./codex-flow batchtools benchmark --detailed --export

# System resource analysis
./codex-flow performance report --system --recommendations
\`\`\`
...
./codex-flow memory batch-store entries.json --parallel

# Store with concurrent validation
./codex-flow memory concurrent-store "multiple_keys" "values" --namespace arch --validate

# Bulk storage with optimization
./codex-flow memory bulk-store project-data/ --recursive --optimize --parallel
\`\`\`
...
./codex-flow memory parallel-query "auth,api,database" --concurrent

# Search across multiple namespaces simultaneously
./codex-flow memory concurrent-search "authentication" --namespaces arch,impl,test --parallel

# Batch query processing
./codex-flow memory batch-query queries.json --optimize --results-parallel
\`\`\`
...
./codex-flow memory stats --real-time --performance

# Concurrent analysis across all namespaces
./codex-flow memory concurrent-stats --all-namespaces --detailed

# Batch performance analysis
./codex-flow memory performance-stats --optimization --benchmarks
\`\`\`
...
./codex-flow memory concurrent-export namespaces.json --parallel --compress

# Batch import with validation
./codex-flow memory batch-import backups/ --validate --parallel

# Incremental export with optimization
./codex-flow memory incremental-export --since yesterday --optimize --concurrent
\`\`\`
...
./codex-flow memory concurrent-cleanup --namespaces temp,cache --days 7 --parallel

# Smart cleanup with optimization
./codex-flow memory smart-cleanup --auto-optimize --performance-based

# Batch maintenance operations
./codex-flow memory batch-maintenance --compress --reindex --parallel
\`\`\`
...
./codex-flow memory batch-store sparc-contexts.json --namespace sparc --parallel

# Concurrent storage across multiple namespaces
./codex-flow memory concurrent-store spec,arch,impl "project data" --parallel --validate

# Performance-optimized bulk storage
./codex-flow memory bulk-store project-data/ --optimize --concurrent --compress
\`\`\`

### Query project decisions with concurrent processing:
\`\`\`bash
# Parallel queries across multiple namespaces
./codex-flow memory parallel-query "authentication" --namespaces arch,impl,test --concurrent

# Batch query processing with optimization
./codex-flow memory batch-query project-queries.json --optimize --results-concurrent

# Real-time search with performance monitoring
./codex-flow memory concurrent-search "API design" --real-time --performance
\`\`\`

### Backup project memory with parallel processing:
\`\`\`bash
# Concurrent export with compression
./codex-flow memory concurrent-export project-$(date +%Y%m%d).json --namespace project --compress --parallel

# Batch backup with incremental processing
./codex-flow memory batch-backup --incremental --all-namespaces --optimize

# Performance-optimized full backup
./codex-flow memory parallel-backup --full --compress --validate --concurrent
\`\`\`
...
./codex-flow memory debug --performance --concurrent

# Analyze batch operation efficiency
./codex-flow memory analyze --batchtools --optimization

# Check parallel processing status
./codex-flow memory status --parallel --detailed
\`\`\`

### Optimization Commands
\`\`\`bash
# Optimize memory configuration
./codex-flow memory optimize --auto-tune --performance

# Benchmark memory operations
./codex-flow memory benchmark --all-operations --detailed

# Performance report generation
./codex-flow memory performance-report --detailed --recommendations
\`\`\`
...
\`\`\`bash
./codex-flow swarm "your complex task" --strategy <type> [options] --parallel
\`\`\`
...
./codex-flow memory batch-store swarm-contexts.json --namespace swarm --parallel

# Concurrent query across swarm namespaces
./codex-flow memory parallel-query "swarm_coordination" --namespaces swarm,agents,tasks --concurrent

# Performance-optimized swarm memory export
./codex-flow memory concurrent-export swarm-backup.json --namespace swarm --compress --parallel
\`\`\`
...
\`\`\`bash
./codex-flow swarm "Test task" --dry-run --strategy development --parallel --performance
\`\`\`
...
./codex-flow swarm debug --performance --concurrent --verbose

# Analyze batch operation efficiency
./codex-flow swarm analyze --batchtools --optimization --detailed

# Check parallel processing status
./codex-flow swarm status --parallel --performance --real-time
\`\`\`

### Optimization Commands
\`\`\`bash
# Auto-optimize swarm configuration
./codex-flow swarm optimize --auto-tune --performance

# Performance benchmarking
./codex-flow swarm benchmark --all-strategies --detailed

# Resource usage analysis
./codex-flow swarm resources --concurrent --optimization
\`\`\`
