...
npx codex-flow sparc run ${mode.slug} "${getExampleTask(mode.slug)}"

# For alpha features
npx codex-flow@alpha sparc run ${mode.slug} "${getExampleTask(mode.slug)}"

# With namespace
npx codex-flow sparc run ${mode.slug} "your task" --namespace ${mode.slug}

# Non-interactive mode
npx codex-flow sparc run ${mode.slug} "your task" --non-interactive
\`\`\`

### Option 3: Local Installation
\`\`\`bash
# If codex-flow is installed locally
./codex-flow sparc run ${mode.slug} "${getExampleTask(mode.slug)}"
\`\`\`
...
// Query previous work
mcp__codex-flow__memory_search {
  pattern: "${mode.slug}",
  namespace: "${mode.slug}",
  limit: 5
}
...
\`\`\`bash
# Store mode-specific context
npx codex-flow memory store "${mode.slug}_context" "important decisions" --namespace ${mode.slug}

# Query previous work
npx codex-flow memory query "${mode.slug}" --limit 5
\`\`\`
`;
}
