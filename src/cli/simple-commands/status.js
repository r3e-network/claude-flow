...
function displayStatus(status, verbose) {
  ...
  if (!status.orchestrator.running) {
    console.log('   Run "codex-flow start" to begin orchestration');
  }
  if (status.agents.active === 0) {
    console.log('   Run "codex-flow agent spawn researcher" to create an agent');
  }
  if (status.memory.entries === 0) {
    console.log('   Run "codex-flow memory store key value" to test memory');
  }
}
