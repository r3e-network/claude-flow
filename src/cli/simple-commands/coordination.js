...
async function swarmInitCommand(subArgs, flags) {
  ...
  console.log(`\n📋 NEXT STEPS:`);
  console.log(
    `  1. Spawn agents: codex-flow coordination agent-spawn --type <type> --swarm-id ${swarmId}`,
  );
  console.log(
    `  2. Orchestrate tasks: codex-flow coordination task-orchestrate --task "<description>" --swarm-id ${swarmId}`,
  );
  console.log(`  3. Monitor swarm: codex-flow monitoring swarm-monitor --swarm-id ${swarmId}`);
}
