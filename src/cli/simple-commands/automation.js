...
async function runWorkflowCommand(subArgs, flags) {
  ...
  if (!workflowFile) {
    ...
    console.log('\nExample:');
    console.log('  codex-flow automation run-workflow workflow.json --claude --non-interactive');
    return;
  }
...
EXAMPLES:
  ...
  codex-flow automation run-workflow my-workflow.json --claude --non-interactive

  # Run MLE-STAR ML engineering workflow (flagship command) - non-interactive by default
  codex-flow automation mle-star --dataset data/train.csv --target price --claude

  # MLE-STAR with custom configuration
  codex-flow automation mle-star --dataset sales.csv --target revenue --output models/sales/ --name "sales-prediction" --search-iterations 5
  
  # MLE-STAR with interactive mode (single Claude coordinator)
  codex-flow automation mle-star --dataset data.csv --target label --claude --interactive
  
  # MLE-STAR with stream-json chaining (agents pipe outputs to each other)
  codex-flow automation mle-star --dataset data.csv --target label --claude --output-format stream-json
  
  # MLE-STAR with minimal output for CI/CD pipelines
  codex-flow automation mle-star --dataset data.csv --target label --claude --quiet
  
  # Custom workflow with stream chaining enabled
  codex-flow automation run-workflow analysis-pipeline.json --claude --output-format stream-json
  
  # Disable chaining for independent task execution
  codex-flow automation mle-star --dataset data.csv --target label --claude --no-chaining
  
  # View stream chaining in action with verbose output
  codex-flow automation mle-star --dataset data.csv --target label --claude --verbose
