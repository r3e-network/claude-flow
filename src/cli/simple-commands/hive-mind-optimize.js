...
${chalk.bold('USAGE:')}
  codex-flow hive-mind-optimize [options]
...
${chalk.bold('EXAMPLES:')}
  ...
  codex-flow hive-mind-optimize

  ${chalk.gray('# Auto-optimize with all features')}
  codex-flow hive-mind-optimize --auto --vacuum --clean-memory --archive-tasks

  ${chalk.gray('# Generate report only')}
  codex-flow hive-mind-optimize --report

  ${chalk.gray('# Custom retention periods')}
  codex-flow hive-mind-optimize --clean-memory --memory-days 60 --task-days 14
...
async function runOptimization(dbPath, options) {
  ...
  console.log('  • Monitor performance with: codex-flow hive-mind metrics');
  console.log('  • Schedule regular maintenance: codex-flow hive-mind-optimize --auto');
  console.log('  • Check swarm status: codex-flow hive-mind status');
}
...
async function generateReport(dbPath) {
  ...
  if (report.schemaVersion >= 1.5) {
    ...
    console.log(chalk.gray('    Run: codex-flow hive-mind-optimize'));
  }
