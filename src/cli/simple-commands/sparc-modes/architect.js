...
1. **Requirements Analysis** (10 mins)
   ...
   - Query existing project context: \`npx codex-flow memory query ${memoryNamespace}\`
   ...
   - Store findings: \`npx codex-flow memory store ${memoryNamespace}_requirements "Core components: X, Y, Z. External deps: API-A, Service-B. Security: OAuth2, RLS policies needed."\`
...
2. **System Architecture Design** (20 mins)
   ...
   - Store architecture: \`npx codex-flow memory store ${memoryNamespace}_architecture "Microservices: auth-service, user-service, data-processor. APIs: REST for external, gRPC for internal. State: Event-sourced with CQRS."\`
...
3. **Technical Specifications** (15 mins)
   ...
   - Store specs: \`npx codex-flow memory store ${memoryNamespace}_tech_specs "Auth: JWT with refresh tokens. DB: PostgreSQL with read replicas. Cache: Redis. Config: Environment-based with secrets manager."\`
...
4. **Modular Implementation Plan** (10 mins)
   ...
   - Store plan: \`npx codex-flow memory store ${memoryNamespace}_implementation_plan "Phase 1: Core auth (tdd mode). Phase 2: User management (code mode). Phase 3: Integration (integration mode)."\`
...
- \`npx codex-flow sparc run spec-pseudocode "Create detailed pseudocode for ${taskDescription}" --non-interactive\`
- \`npx codex-flow sparc run security-review "Review architecture for vulnerabilities" --non-interactive\`

### Background Swarm Mode (for complex implementation):
- \`npx codex-flow swarm "Implement complete ${taskDescription} based on architecture" --strategy development --background --monitor\`
- \`npx codex-flow swarm "Create comprehensive test suite for ${taskDescription}" --strategy testing --background --parallel\`
- \`npx codex-flow swarm "Generate complete documentation for ${taskDescription}" --strategy research --background\`
...
npx codex-flow swarm "Research and analyze all architectural patterns for ${taskDescription}" \\
...
npx codex-flow swarm "Create architectural proof of concepts and prototypes for ${taskDescription}" \\
...
npx codex-flow swarm "Generate complete technical specifications and API documentation for ${taskDescription}" \\
...
npx codex-flow sparc run architect "Initial system design for ${taskDescription}" --non-interactive
npx codex-flow swarm "Refine architecture based on security and performance analysis" \\
...
npx codex-flow sparc run architect "Core system design" --non-interactive

# Phase 2: Parallel background research (Swarm)
npx codex-flow swarm "Deep dive research into scalability patterns" --strategy research --background &
npx codex-flow swarm "Security analysis and threat modeling" --strategy analysis --background &
npx codex-flow swarm "Performance benchmarking and optimization research" --strategy optimization --background &

# Phase 3: Implementation swarm (Background)
npx codex-flow swarm "Full implementation of ${taskDescription} architecture" \\
