...
1. **Requirements Analysis & MCP Setup** (10 mins)
   ...
   - Query existing project context:
     ...
     npx codex-flow memory query ${memoryNamespace}_data_models
     npx codex-flow memory query ${memoryNamespace}_auth_requirements
     \`\`\`
   ...
   - Store analysis: \`npx codex-flow memory store ${memoryNamespace}_supabase_requirements "Requirements: ${taskDescription}. Tables needed: users, profiles, permissions. Auth: Email + OAuth. Storage: user avatars, documents."\`
...
2. **Database Schema Design** (15 mins)
   ...
   - Store design: \`npx codex-flow memory store ${memoryNamespace}_db_design "Tables: users (id, email, created_at), profiles (user_id, full_name, avatar_url), roles (id, name), user_roles (user_id, role_id). Indexes: users_email_idx, profiles_user_id_idx."\`
...
3. **Schema Implementation via MCP** (20 mins)
   ...
   - Set up audit trails
   - Store migrations: \`npx codex-flow memory store ${memoryNamespace}_migrations "Created: 5 tables, 8 indexes, 3 functions, 2 triggers. Branch: dev-${Date.now()}. Ready for RLS policies."\`
...
4. **Security & RLS Implementation** (15 mins)
   ...
   - Create security functions:
     ...
     \`\`\`
   - Store security config: \`npx codex-flow memory store ${memoryNamespace}_security "RLS enabled on all tables. Policies: 15 created (CRUD + special cases). Auth providers: email, google. Custom claims: user_role, organization_id."\`
...
5. **API & Storage Configuration** (10 mins)
   ...
   - Store completion: \`npx codex-flow memory store ${memoryNamespace}_supabase_complete "Supabase setup complete. API URL: https://xxx.supabase.co. Types generated. Storage buckets: avatars (public), documents (authenticated). Ready for integration."\`
...
- \`npx codex-flow sparc run code "Integrate Supabase client in application" --non-interactive\`
- \`npx codex-flow sparc run tdd "Write tests for Supabase operations" --non-interactive\`
- \`npx codex-flow sparc run docs-writer "Create Supabase integration guide" --non-interactive\``;
}
