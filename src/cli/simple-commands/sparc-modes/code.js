...
1. **Project Directory Setup & Context Review** (5 mins)
   ...
   - Query architecture and pseudocode: 
     \`\`\`bash
     npx codex-flow memory query ${memoryNamespace}_architecture
     npx codex-flow memory query ${memoryNamespace}_pseudocode
     npx codex-flow memory query ${memoryNamespace}_tech_specs
     \`\`\`
...
2. **Project Setup & Configuration** (10 mins)
   ...
   - Store setup: \`npx codex-flow memory store ${memoryNamespace}_setup "Project structure: src/{domain,application,infrastructure}. Config: dotenv + vault integration. Dependencies: express, joi, winston."\`
...
3. **Modular Implementation** (30 mins)
   ...
   - Store progress: \`npx codex-flow memory store ${memoryNamespace}_implementation "Completed: auth-service (3 modules), user-repository (2 modules). Remaining: notification-service."\`
...
4. **Integration & Basic Testing** (15 mins)
   ...
   - Store results: \`npx codex-flow memory store ${memoryNamespace}_tests "Smoke tests passing. Health checks operational. Error handling verified. Ready for TDD mode deep testing."\`
...
5. **Code Quality & Documentation** (10 mins)
   ...
   - Store completion: \`npx codex-flow memory store ${memoryNamespace}_code_complete "Implementation complete. All modules < 500 lines. No hardcoded secrets. Ready for testing and integration."\`
...
- \`npx codex-flow sparc run tdd "Write comprehensive tests for ${taskDescription}" --non-interactive\`
- \`npx codex-flow sparc run security-review "Security audit for ${taskDescription}" --non-interactive\`

### Background Swarm Mode (for complex comprehensive tasks):
- \`npx codex-flow swarm "Create comprehensive test suite with unit, integration, and e2e tests for ${taskDescription}" --strategy testing --background --parallel\`
- \`npx codex-flow swarm "Perform full security audit and penetration testing for ${taskDescription}" --strategy analysis --background --review\`
- \`npx codex-flow swarm "Generate complete documentation, API specs, and deployment guides for ${taskDescription}" --strategy research --background\`
...
npx codex-flow swarm "Implement complete ${taskDescription} with all modules, testing, and documentation" \\
...
npx codex-flow swarm "Comprehensive QA including testing, security review, and performance optimization for ${taskDescription}" \\
  --strategy testing --background --review --monitor \\
  --output ./qa-results

# Background documentation and deployment swarm
npx codex-flow swarm "Create production-ready documentation, deployment scripts, and monitoring for ${taskDescription}" \\
  --strategy research --background --parallel \\
  --output ./deployment-ready

# Monitor all background development
npx codex-flow status
npx codex-flow monitor
\`\`\`
...
npx codex-flow sparc run code "Core ${taskDescription} prototype" --non-interactive

# Phase 2: Parallel background development (Swarm)
npx codex-flow swarm "Full implementation of ${taskDescription} with advanced features" --strategy development --background --parallel &
npx codex-flow swarm "Comprehensive testing strategy for ${taskDescription}" --strategy testing --background &
npx codex-flow swarm "Security hardening and performance optimization" --strategy optimization --background &

# Phase 3: Integration and deployment (Background)
npx codex-flow swarm "Integration testing and production deployment preparation for ${taskDescription}" \\
