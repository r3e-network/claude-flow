...
1. **Security Audit Scope Definition** (10 mins)
   ...
   - Query system architecture and sensitive areas:
     ...
     npx codex-flow memory query ${memoryNamespace}_auth
     npx codex-flow memory query ${memoryNamespace}_config
     \`\`\`
   ...
   - Store scope: \`npx codex-flow memory store ${memoryNamespace}_security_scope "Audit scope: ${taskDescription}. Critical assets: user PII, JWT tokens, payment data. External interfaces: REST API, webhooks, third-party integrations."\`
...
2. **Static Security Analysis** (20 mins)
   ...
   - Review dependency vulnerabilities:
     ...
     npm audit
     \`\`\`
   - Store findings: \`npx codex-flow memory store ${memoryNamespace}_vulnerabilities "Critical: 2 hardcoded API keys found. High: Missing input validation in user-controller. Medium: Outdated JWT library. Low: Verbose error messages expose stack traces."\`
...
3. **Dynamic Security Analysis** (10 mins)
   ...
   - Store dynamic findings: \`npx codex-flow memory store ${memoryNamespace}_dynamic_findings "Auth bypass: None found. Rate limiting: Missing on login endpoint. CORS: Overly permissive. Error handling: Leaks database schema in dev mode."\`
...
4. **Risk Assessment & Prioritization** (15 mins)
   ...
   - Store assessment: \`npx codex-flow memory store ${memoryNamespace}_risk_assessment "Critical risks: 2 (hardcoded secrets, missing auth on admin endpoints). High risks: 3 (outdated deps, missing rate limiting, weak session management). Compliance impact: GDPR violation risk due to logging PII."\`
...
5. **Remediation Plan & Implementation** (10 mins)
   ...
   - Store plan: \`npx codex-flow memory store ${memoryNamespace}_remediation_plan "Immediate: Remove hardcoded secrets, fix auth bypass. Next sprint: Implement rate limiting, update dependencies. Future: Add WAF, implement SIEM integration."\`
...
- \`npx codex-flow sparc run code "Implement security remediation plan" --non-interactive\`
- \`npx codex-flow sparc run tdd "Write security test cases" --non-interactive\`
- \`npx codex-flow sparc run post-deployment-monitoring-mode "Set up security monitoring and alerts" --non-interactive\``;
}
