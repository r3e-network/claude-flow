...
1. **Issue Analysis & Reproduction** (10 mins)
   ...
   - Query relevant context and previous issues:
     \`\`\`bash
     npx codex-flow memory query ${memoryNamespace}_bugs
     npx codex-flow memory query ${memoryNamespace}_implementation
     npx codex-flow memory query ${memoryNamespace}_tests
     \`\`\`
   ...
   - Store findings: \`npx codex-flow memory store ${memoryNamespace}_issue_analysis "Issue: ${taskDescription}. Reproduction: Steps 1-3. Error: Stack trace at line X. Affected: auth-service, user-repository."\`
...
2. **Root Cause Investigation** (20 mins)
   ...
   - Store investigation: \`npx codex-flow memory store ${memoryNamespace}_root_cause "Root cause: Race condition in auth token refresh. Occurs under high load when token expires during request processing. Fix approach: Implement token refresh mutex."\`
...
3. **Solution Development** (15 mins)
   ...
   - Store solution: \`npx codex-flow memory store ${memoryNamespace}_solution "Fix: Added mutex lock for token refresh. Tests: 3 regression tests added. Validation: Added token expiry check. No breaking changes."\`
...
4. **Validation & Performance Check** (10 mins)
   ...
   - Store validation: \`npx codex-flow memory store ${memoryNamespace}_debug_validation "All tests passing. Performance impact: <5ms latency increase. Memory usage unchanged. Original issue resolved."\`
...
- \`npx codex-flow sparc run security-review "Review security impact of ${taskDescription} fix" --non-interactive\`
- \`npx codex-flow sparc run refinement-optimization-mode "Refactor affected modules for better maintainability" --non-interactive\`
- \`npx codex-flow sparc run post-deployment-monitoring-mode "Monitor ${taskDescription} fix in production" --non-interactive\``;
}
