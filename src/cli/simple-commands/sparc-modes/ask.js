...
- **\`spec-pseudocode\`** - For creating detailed specifications, requirements, and high-level logic
  ...
  - Example: \`npx codex-flow sparc run spec-pseudocode "Create user authentication flow" --non-interactive\`
...
- **\`architect\`** - For system design, API boundaries, and service architecture
  ...
  - Example: \`npx codex-flow sparc run architect "Design e-commerce platform architecture" --non-interactive\`

#### 🧠 Implementation Modes:
- **\`code\`** - For implementing features with clean architecture
  - Use when: Building new features, writing production code
  - Example: \`npx codex-flow sparc run code "Implement payment processing service" --non-interactive\`
...
- **\`tdd\`** - For test-first development with comprehensive coverage
  ...
  - Example: \`npx codex-flow sparc run tdd "Build shopping cart with TDD" --non-interactive\`
...
- **\`debug\`** - For troubleshooting issues and fixing bugs
  ...
  - Example: \`npx codex-flow sparc run debug "Fix memory leak in data processor" --non-interactive\`
...
- **\`security-review\`** - For security audits and vulnerability checks
  ...
  - Example: \`npx codex-flow sparc run security-review "Audit user data handling" --non-interactive\`
...
- **\`refinement-optimization-mode\`** - For performance optimization and refactoring
  ...
  - Example: \`npx codex-flow sparc run refinement-optimization-mode "Optimize database queries" --non-interactive\`

#### 📚 Documentation & Integration:
- **\`docs-writer\`** - For creating documentation and guides
  - Use when: Need README, API docs, user guides
  - Example: \`npx codex-flow sparc run docs-writer "Create API documentation" --non-interactive\`
...
- **\`integration\`** - For connecting services and ensuring system cohesion
  ...
  - Example: \`npx codex-flow sparc run integration "Connect auth service to user service" --non-interactive\`
...
- **\`devops\`** - For deployment, CI/CD, and infrastructure
  ...
  - Example: \`npx codex-flow sparc run devops "Deploy to AWS with auto-scaling" --non-interactive\`
...
- **\`post-deployment-monitoring-mode\`** - For production monitoring
  ...
  - Example: \`npx codex-flow sparc run post-deployment-monitoring-mode "Setup DataDog monitoring" --non-interactive\`
...
- **\`supabase-admin\`** - For Supabase database and auth management
  ...
  - Example: \`npx codex-flow sparc run supabase-admin "Setup user authentication database" --non-interactive\`

- **\`mcp\`** - For MCP (Model Context Protocol) integrations
  - Use when: Integrating external tools via MCP, API connections
  - Example: \`npx codex-flow sparc run mcp "Integrate Stripe payment API" --non-interactive\`
...
npx codex-flow sparc run spec-pseudocode "${taskDescription} - requirements and pseudocode" --non-interactive

# Step 2: Design architecture (if needed)
npx codex-flow sparc run architect "${taskDescription} - system design" --non-interactive

# Step 3: Implement with TDD
npx codex-flow sparc run tdd "${taskDescription} - implementation" --non-interactive

# Step 4: Security review
npx codex-flow sparc run security-review "${taskDescription} - security audit" --non-interactive

# Step 5: Documentation
npx codex-flow sparc run docs-writer "${taskDescription} - documentation" --non-interactive
\`\`\`
...
npx codex-flow memory store ${memoryNamespace}_context "Working on: ${taskDescription}"
