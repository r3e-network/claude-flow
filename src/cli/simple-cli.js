...
🎮 ENTERPRISE QUICK START:
  ...
  npx codex-flow@2.0.0 init --sparc
...
async function main() {
  ...
  if (
    ...
    if (detailedHelp && !detailedHelp.includes('Help not available')) {
      ...
      console.log('\nRun "codex-flow --help" to see available commands.');
    }
    return;
  }
  ...
  if (hasCommand(command)) {
    try {
      ...
      console.log(`\nRun "codex-flow ${command} --help" for usage information.`);
      return;
    }
  }
...
`);
    ...
    config: async (key) => {
      try {
        const config = JSON.parse(await readTextFile('codex-flow.config.json'));
        if (key) {
