...
1. Run directly: `npx codex-flow sparc run debug "your task"`
2. Use in workflow: Include `debug` in your SPARC workflow
3. Delegate tasks: Use `new_task` to assign work to this mode
4. Batch operations: `npx codex-flow sparc run debug --batch "debug auth issues across services"`

## Example

```bash
# Standard debugging
npx codex-flow sparc run debug "fix authentication timeout errors"

# Batch debugging across services
npx codex-flow sparc run debug --batch "debug all microservice communication issues"

# Parallel log analysis
npx codex-flow sparc run debug --parallel-logs "analyze all error logs from last deployment"
```
