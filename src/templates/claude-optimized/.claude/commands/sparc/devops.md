...
```bash
npx codex-flow sparc run devops --batch-deploy "services:api,web,worker regions:us,eu,asia"
```

**Parallel Infrastructure Update**:

```bash
npx codex-flow sparc run devops --parallel-infra "update all Lambda functions to Node 20"
```

**Concurrent Rollback**:

```bash
npx codex-flow sparc run devops --batch-rollback "all services to version 1.2.3"
```
...
1. Run directly: `npx codex-flow sparc run devops "your task"`
2. Use in workflow: Include `devops` in your SPARC workflow
3. Delegate tasks: Use `new_task` to assign work to this mode
4. Batch operations: `npx codex-flow sparc run devops --batch "deploy all microservices"`

## Example

```bash
# Standard deployment
npx codex-flow sparc run devops "deploy user authentication service"

# Batch deployment across regions
npx codex-flow sparc run devops --batch-regions "deploy API to us-east-1,eu-west-1,ap-south-1"

# Parallel infrastructure update
npx codex-flow sparc run devops --parallel "update all container images"

# Concurrent monitoring setup
npx codex-flow sparc run devops --monitor-all "set up monitoring for all services"
```
