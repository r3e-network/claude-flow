...
npx codex-flow sparc run code "test batch operations" --non-interactive
```
...
npx codex-flow memory export pre-migration-backup.json
...
npx codex-flow sparc run architect "design user service"
# Wait...
npx codex-flow sparc run architect "design auth service"
# Wait...
npx codex-flow sparc run architect "design API gateway"
```
...
npx codex-flow sparc run architect "design complete microservices architecture with user, auth, and gateway services"
...
npx codex-flow sparc run code "create user controller"
npx codex-flow sparc run code "create user service"
npx codex-flow sparc run code "create user repository"
```
...
npx codex-flow sparc run code "implement complete user management with controller, service, repository, and tests"
...
```bash
# Old
npx codex-flow sparc run code "create user API"

# New with options
npx codex-flow sparc run code "create user API" --non-interactive --parallel --batch-size=10
```
...
if npx codex-flow sparc run code "test parallel" --non-interactive --dry-run 2>&1 | grep -q "parallel"; then
    echo "✅"
else
...
if npx codex-flow memory stats > /dev/null 2>&1; then
    echo "✅"
else
...
time npx codex-flow sparc run code "create test component" --non-interactive > /dev/null 2>&1
...
if [ -f "pre-migration-backup.json" ]; then
    npx codex-flow memory import pre-migration-backup.json
fi
...
npx codex-flow sparc run architect "test batch architecture" --non-interactive
npx codex-flow sparc run tdd "test parallel testing" --non-interactive
npx codex-flow sparc run code "test concurrent generation" --non-interactive
```
...
time npx codex-flow sparc run code "create user CRUD" --legacy

echo "Testing new method..."
time npx codex-flow sparc run code "create user CRUD" --non-interactive
```
...
```bash
# Test full workflow
npx codex-flow sparc tdd "implement complete feature with batchtools"
```
...
const tests = [
  {
    ...
    old: 'npx codex-flow sparc run code "create user CRUD" --legacy',
    new: 'npx codex-flow sparc run code "create user CRUD with all operations"',
  },
  {
    name: 'Test Suite Creation',
    old: 'npx codex-flow sparc run tdd "create auth tests" --legacy',
    new: 'npx codex-flow sparc run tdd "create complete auth test suite"',
  },
  {
    name: 'Architecture Design',
    old: 'npx codex-flow sparc run architect "design microservices" --legacy',
    new: 'npx codex-flow sparc run architect "design complete microservices architecture"',
  },
];
...
batchtool orchestrate --prototypes \
  --idea-1 "npx codex-flow sparc run code 'social feed prototype'" \
  --idea-2 "npx codex-flow sparc run code 'marketplace prototype'" \
  --idea-3 "npx codex-flow sparc run code 'subscription prototype'" \
  --test-all "npx codex-flow sparc run tdd 'test all prototypes'" \
  --compare "npx codex-flow sparc run architect 'analyze best approach'"
```
...
# Run with batchtools
npx codex-flow sparc run <mode> "<task>" --non-interactive
